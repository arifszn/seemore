...
export function Mermaid({ chart }: { chart: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const { svg: rendered } = await mermaid.render(`openmd-mermaid-${id}`, chart);
  ...
  if (error !== undefined) {
    return (
      <pre className="openmd-mermaid-error" role="note">
  ...
  return (
    ...
    >
      {svg === undefined ? <pre className="openmd-mermaid-source">{chart}</pre> : undefined}
    </div>
  );
}
