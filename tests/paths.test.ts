...
function tmp(): string {
  return realpathSync.native(mkdtempSync(join(tmpdir(), 'seemore-paths-')));
}

describe('resolveContentRoot', () => {
  it('uses the folder the command runs in when no dir is given', () => {
    const cwd = tmp();
    expect(resolveContentRoot(cwd)).toBe(realpathSync.native(cwd));
  });
  ...
  it('resolves an explicit dir against the cwd', () => {
    ...
    expect(resolveContentRoot(cwd, 'docs')).toBe(join(realpathSync.native(cwd), 'docs'));
    expect(resolveContentRoot(cwd, './nested/dir')).toBe(join(realpathSync.native(cwd), 'nested/dir'));
  });

  it('canonicalises the result through the filesystem', () => {
    // tmpdir() is a symlink on macOS; the resolved root is the real spelling.
    const dir = tmp();
    expect(resolveContentRoot('/tmp', dir)).toBe(realpathSync.native(dir));
  });
});
